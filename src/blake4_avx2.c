/*
 * BLAKE4 AVX2 Implementation
 *
 * Optimized compression function using AVX2 intrinsics.
 * This provides high performance on x86-64 processors with AVX2 support.
 *
 * Key optimizations:
 * - 256-bit vectors for 4 x 64-bit parallel operations
 * - Emulated 64-bit rotates using shift pairs
...
/*
 * AVX2 does NOT have native 64-bit rotate, so we emulate using shift pairs.
 * rotr64(x, n) = (x >> n) | (x << (64 - n))
 */

/* Rotate right by 32 (special case: can use shuffle) */
static inline __m256i ror64_32(__m256i x) {
    return _mm256_shuffle_epi32(x, _MM_SHUFFLE(2, 3, 0, 1));
}

/* Rotate right by 24 */
static inline __m256i ror64_24(__m256i x) {
    return _mm256_or_si256(
        _mm256_srli_epi64(x, 24),
        _mm256_slli_epi64(x, 40)
    );
}

/* Rotate right by 16 */
static inline __m256i ror64_16(__m256i x) {
    return _mm256_or_si256(
        _mm256_srli_epi64(x, 16),
        _mm256_slli_epi64(x, 48)
    );
}

/* Rotate right by 63 */
static inline __m256i ror64_63(__m256i x) {
    return _mm256_or_si256(
 ...
 * Uses 256-bit AVX2 vectors (4 x 64-bit).
 *
 ...
 */
static inline void G_AVX2(__m256i *a, __m256i *b, __m256i *c, __m256i *d,
                           __m256i mx, __m256i my) {
    /* a = a + b + mx */
    *a = _mm256_add_epi64(_mm256_add_epi64(*a, *b), mx);
    /* d = rotr(d ^ a, R1) */
    *d = ror64_32(_mm256_xor_si256(*d, *a));
    /* c = c + d */
    *c = _mm256_add_epi64(*c, *d);
    /* b = rotr(b ^ c, R2) */
    *b = ror64_24(_mm256_xor_si256(*b, *c));
    /* a = a + b + my */
    *a = _mm256_add_epi64(_mm256_add_epi64(*a, *b), my);
    /* d = rotr(d ^ a, R3) */
    *d = ror64_16(_mm256_xor_si256(*d, *a));
    /* c = c + d */
    *c = _mm256_add_epi64(*c, *d);
    /* b = rotr(b ^ c, R4) */
    *b = ror64_63(_mm256_xor_si256(*b, *c));
}

/* ============== AVX2 Compression Function ============== */

/*
 * Compression function using AVX2 intrinsics.
 * Uses 256-bit operations to process 4 G functions in parallel.
 */
static void blake4_compress_avx2_impl(const uint64_t cv[8],
    ...
    for (int r = 0; r < 10; r++) {
        ...
        G_AVX2(&row0, &row1, &row2, &row3, mx, my);
