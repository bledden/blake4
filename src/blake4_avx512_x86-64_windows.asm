;
; BLAKE4 AVX-512 Assembly Implementation (Windows MASM)
;
; Hand-optimized compression function for x86-64 with AVX-512.
...
;
; void blake4_compress_avx512_asm_win(const uint64_t cv[8],
...
;                                      uint64_t out[16])
;
blake4_compress_avx512_asm_win PROC
    ...
    vmovq   xmm4, rax
    vpermq  ymm4, ymm4, 0
    vpand   ymm4, ymm4, YMMWORD PTR [MASK_IDX2_WIN]
    ...
    vmovq   xmm4, rax
    vpermq  ymm4, ymm4, 0
    vpand   ymm4, ymm4, YMMWORD PTR [MASK_IDX3_WIN]
    vpxor   ymm3, ymm3, ymm4

    ; Load message block into stack
    vmovdqu ymm8, YMMWORD PTR [r13]
    vmovdqu ymm9, YMMWORD PTR [r13+32]
    vmovdqu ymm10, YMMWORD PTR [r13+64]
    vmovdqu ymm11, YMMWORD PTR [r13+96]
    vmovdqu YMMWORD PTR [rsp+192], ymm8
    vmovdqu YMMWORD PTR [rsp+224], ymm9
    vmovdqu YMMWORD PTR [rsp+256], ymm10
    vmovdqu YMMWORD PTR [rsp+288], ymm11

    ; Oops - need more stack space. Let's use a different approach.
    ; Store message pointer instead
    mov     rdi, r13                ; message pointer

    ; Get SIGMA table address
    lea     r10, BLAKE4_SIGMA_WIN

    ; Round loop
    mov     ecx, 10

round_loop_avx512_win:
    ; Load SIGMA indices
    ...
    vpxorq  ymm3, ymm3, ymm0
    vprorq  ymm3, ymm3, 32
    vpaddq  ymm2, ymm2, ymm3
    vpxorq  ymm1, ymm1, ymm2
    vprorq  ymm1, ymm1, 24
    vpaddq  ymm0, ymm0, ymm1
    ...
    vpermq  ymm1, ymm1, 39h
    vpermq  ymm2, ymm2, 4Eh
    vpermq  ymm3, ymm3, 93h
    ...
    ; Unshuffle
    vpermq  ymm1, ymm1, 93h
    vpermq  ymm2, ymm2, 4Eh
    vpermq  ymm3, ymm3, 39h

    ; Loop
    add     r10, 16
    dec     ecx
    jnz     round_loop_avx512_win

    ; Finalization
    vpxorq  ymm4, ymm0, ymm2
    vpxorq  ymm5, ymm1, ymm3

    vmovdqu ymm6, YMMWORD PTR [r12]
    vmovdqu ymm7, YMMWORD PTR [r12+32]
    vpxorq  ymm6, ymm2, ymm6
    vpxorq  ymm7, ymm3, ymm7
...
blake4_compress_avx512_asm_win ENDP
